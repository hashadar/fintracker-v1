...
if df is not None:
    ...
    if asset_df.empty:
        ...
        if metrics:
            # Main investment position with emphasis (shows value, MoM, YTD)
            complex_emphasis_card(
            ...
            with col1:
                simple_card(
                    ...
                    caption="Investment platforms"
                )
            with col2:
                simple_card(
                    title="Number of Assets",
                    metric=str(metrics['assets']),
                    caption="Investment products"
                )
            with col3:
                simple_card(
                ...
                for (platform, value), col in zip(metrics['latest_platform_breakdown'].items(), platform_cols):
                    with col:
                        simple_card(
                            title=platform,
                            metric=f"£{value:,.2f}",
                            caption=f"{(value/metrics['latest_value']*100):.1f}% of portfolio"
                        )
        else:
