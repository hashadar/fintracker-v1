...
def calculate_asset_type_metrics(df, asset_type):
    ...
    # Get the latest and previous month's data
    ...
    metrics = {
        ...
        'platforms': type_df['Platform'].nunique(),
        'assets': type_df['Asset'].nunique() if 'Asset' in type_df.columns else 0,
        'months_tracked': len(type_df['Month'].unique()),
        'latest_platform_breakdown': latest_df.groupby('Platform')['Value'].sum().to_dict()
    }
    return metrics
...
def calculate_overall_metrics(df):
    ...
    return {
        ...
        'unique_combinations': len(latest_df)
    }
...
def calculate_allocation_metrics(df):
    ...
    latest_month = df['Month'].max()
    prev_month = df[df['Month'] < latest_month]['Month'].max()
    # YTD: first month of the latest year in the filtered data
    latest_year = latest_month.year
    ytd_start_month = df[df['Month'].dt.year == latest_year]['Month'].min()
    latest_df = df[df['Month'] == latest_month]
    prev_df = df[df['Month'] == prev_month] if pd.notna(prev_month) else None
