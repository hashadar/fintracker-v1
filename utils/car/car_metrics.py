...
def get_latest_valuation(valuations_df, car_id):
    ...
    # Convert to datetime and sort by date to get the latest valuation
...
def calculate_outstanding_finance(finance_agreement, payments_df):
    ...
    # Sum all payments made against this finance agreement
...
def get_key_dates(key_dates_df, car_id):
    ...
    # Calculate days remaining until each key date
...
def get_all_car_metrics(data, car_id):
    ...
    # Filter all related data for this specific car
    ...
    # --- Core Calculations ---
    # Get latest valuation and mileage (fallback to initial if no valuations)
    ...
    equity = latest_valuation - outstanding_finance  # Current equity in the car
    
    # Calculate total costs and contributions
    ...
    # Calculate depreciation (purchase price vs current value)
    pnl_metrics['depreciation'] = purchase_price - latest_valuation if latest_valuation > 0 else 0
    
    # Calculate interest paid (simplified model: total payments - principal paid)
    ...
    # Total cost of ownership: all cash outlay (contributions + running costs)
    pnl_metrics['total_cost_of_ownership'] = total_contribution + total_costs_dict.get('total_cost', 0)

    # Net position: current equity vs total contribution (positive = profit, negative = loss)
    ...
    # Calculate equity percentage (what % of current value is equity)
...
def calculate_cumulative_costs_over_time(finance_agreement, payments_df, expenses_df, car_id):
    ...
    # Get initial purchase contributions (deposit and part exchange)
    contributions = []
    ...
    # Get all running costs (expenses)
    ...
    # Calculate cumulative sums for each transaction type
    ...
    # Forward fill to carry cumulative values forward (so each row shows totals up to that date)
    all_transactions.fillna(method='ffill', inplace=True)
    all_transactions.fillna(0, inplace=True)  # Fill any initial NaNs with 0
    
    # Calculate grand total outlay (contributions + running costs)
